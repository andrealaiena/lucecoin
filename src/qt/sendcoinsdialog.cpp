...
void SendCoinsDialog::updateFeeSectionControls()
{
    ...
    // lucecoin: We don't use smart fees in the UI, so don't need to warn they're not available
...
void SendCoinsDialog::updateGlobalFeeVariables()
{
    if (ui->radioSmartFee->isChecked())
    {
        ...
        // lucecoin: We manually set height well past the last hard fork here
        ui->confirmationTargetLabel->setText(GetlucecoinPriorityLabel(nPriority).c_str());
    }
    else
    {
...
void SendCoinsDialog::updateFeeLabel()
{
    ...
    int nPriority = ui->sliderSmartFee->value();
    CFeeRate feeRate = GetlucecoinFeeRate(nPriority);
    ...
    else
    {
        ...
        // lucecoin: We don't use smart fees, so we don't have the data to estimate when it will get in
        ui->labelFeeEstimation->setText("");
...
void SendCoinsDialog::coinControlChangeEdited(const QString& text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid lucecoin address"));
        }
        else // Valid address
        {
            CKeyID keyid;
