...
#if defined(NDEBUG)
# error "lucecoin cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "lucecoin Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (fLimitFree && nModifiedFees < GetlucecoinMinRelayFee(tx, nSize, !fLimitFree))
        {
...
bool CheckTxInputs(const CChainParams& params, const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
        ...
        for (unsigned int i = 0; i < tx.vin.size(); i++)
        {
            ...
            if (coins->IsCoinBase()) {
                // lucecoin: Switch maturity at depth 145,000
...
void ThreadScriptCheck() {
    RenameThread("lucecoin-scriptch");
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // lucecoin: BIP30 has been active since inception
    ...
    // lucecoin: BIP16 has been enabled since inception
    ...
    CAmount blockReward = nFees + GetlucecoinBlockSubsidy(pindex->nHeight, chainparams.GetConsensus(pindex->nHeight), hashPrevBlock);
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // lucecoin: Disallow AuxPow blocks before it is activated.
    ...
    // lucecoin: Version 2 enforcement was never used
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindexPrev)
{
    ...
    // lucecoin: We probably want to disable this
    int nLockTimeFlags = 0;
