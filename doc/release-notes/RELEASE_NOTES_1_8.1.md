...
lucecoin Core 1.8.1 is primarily a bugfix release, bringing lucecoin Core in
line with Bitcoin 0.9.3. lucecoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by lucecoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 lucecoin Core clients
* Use HTTPS with LUCEchain.info
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/lucecoin/lucecoin/issues/134).
...
This only affects values significantly below 1 LUCE, however merchants may wish to round to
the nearest LUCE on all transactions for now.
