...
Below are some notes on how to build lucecoin Core for Windows.
...
To build lucecoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the lucecoin GitHub repository](https://github.com/lucecoin/lucecoin/)).
...
Note that for WSL v1 the lucecoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/lucecoin, AND not under, for example, /mnt/d/lucecoin.
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for lucecoin's dependencies):
...
way. This will install to `c:\workspace\lucecoin`, for example:

    make install DESTDIR=/mnt/c/workspace/lucecoin
