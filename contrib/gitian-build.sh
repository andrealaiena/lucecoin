...
# Dependencies
ossPatchUrl="https://depends.lucecoincore.org/osslsigncode-Backports-to-1.7.1.patch"
ossPatchHash="a8c4e9cafba922f89de0df1f2152e7be286aba73f78505169bc351a7938dd911"

ossTarUrl="https://depends.lucecoincore.org/osslsigncode_1.7.1.orig.tar.gz"
ossTarHash="f9a8cdb38b9c309326764ebc937cba1523a3a751a7ab05df3ecc99d18ae466c9"

macosSdkUrl="https://depends.lucecoincore.org/MacOSX10.11.sdk.tar.gz"
macosSdkHash="bec9d089ebf2e2dd59b1a811a38ec78ebd5da18cbbcd6ab39d1e59f64ac5033f"

liefUrl="https://depends.lucecoincore.org/lief-0.12.3-cp38-cp38-manylinux_2_17_x86_64.manylinux2014_x86_64.whl"
...
SIGNER=
VERSION=
url=https://github.com/lucecoin/lucecoin
proc=2
mem=2000
...
Standalone script to perform the gitian build of lucecoin Core. Perform
...
-u|--url repo       Specify the URL of the repository. Default is https://github.com/lucecoin/lucecoin
...
function move_build_files() {
    find build/out -type f -exec mv '{}' $outputDir/lucecoin-binaries/${VERSION}/ \;
}
...
if [[ $setup == true ]]; then
    git clone https://github.com/lucecoin/gitian.sigs.git
    git clone https://github.com/lucecoin/lucecoin-detached-sigs.git
...
if [[ $build == true ]]; then
    # Make output folder
    mkdir -p $outputDir/lucecoin-binaries/"$VERSION"

    pushd ./gitian-builder || exit 1

    # Clean lucecoin git directory because of old caching
    if [ -d inputs/lucecoin/ ]; then
        echo "Cleaning lucecoin directory..."
        rm -rf inputs/lucecoin/
    fi
    ...
    for descriptor in "${DESCRIPTORS[@]}"; do
        ...
        echo ""
        ./bin/gbuild -j "$proc" -m "$mem" --commit lucecoin="$COMMIT" --url lucecoin="$url" ../gitian-descriptors/gitian-"$descriptor".yml  || exit 1
